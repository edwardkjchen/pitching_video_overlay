...
This version (v04) adds a scale alignment step to the pipeline.
...
3.  **Spatial Alignment**: It calculates the spatial displacement required to
...
def main():
    ...
    for i in range(len(video_files) - 1):
        ...
        stabilized1_path = os.path.join(output_dir, f"{base1}_stabilized_v04.mp4")
        stabilized2_path = os.path.join(output_dir, f"{base2}_stabilized_v04.mp4")

        scaled1_path = os.path.join(output_dir, f"{base1}_scaled_v04.mp4")
        scaled2_path = os.path.join(output_dir, f"{base2}_scaled_v04.mp4")
        
        trimmed1_path = os.path.join(output_dir, f"{base1}_trimmed_v04.mp4")
        trimmed2_path = os.path.join(output_dir, f"{base2}_trimmed_v04.mp4")
        
        final_overlay_path = os.path.join(output_dir, f"{base1}_overlay_{base2}_v04.mp4")
        ...
        stabilize_video(video1_path, stabilized1_path, 'motion_raw1_v04.csv', 'motion_smoothed1_v04.csv')
        stabilize_video(video2_path, stabilized2_path, 'motion_raw2_v04.csv', 'motion_smoothed2_v04.csv')
        ...
        print("\nStep 3: Starting spatial alignment...")
        # Use the scaled videos to get the correct displacement
        displacement = get_spatial_displacement(scaled1_path, scaled2_path, debug_output_dir=debug_dir)
        print("Spatial alignment complete.")

        # Step 4: Temporal Alignment
        print("\nStep 4: Starting temporal alignment...")
        ...
        # Step 5: Trim videos based on the calculated shift
        print("\nStep 5: Trimming videos for synchronization...")
        if frame_shift > 0:
            ...
            trim_video(scaled2_path, trimmed2_path, abs(frame_shift))
            shutil.copy(scaled1_path, trimmed1_path)
        elif frame_shift < 0:
            ...
            trim_video(scaled1_path, trimmed1_path, abs(frame_shift))
            shutil.copy(scaled2_path, trimmed2_path)
        else:
        ...
        print("\nStep 6: Rendering final overlay...")
